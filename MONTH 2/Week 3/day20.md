...
```
`section_index` is the new, central idea today. a simple counter tracking *which* of the five topics the system is currently working on. Everything else here should look familiar: `research_packets`, `report_sections`, and `error_log` are the same shapes of data I built on Days 18 and 19, just now living inside a state designed to move through several topics rather than just one.
